...
$.fn.htmlbox=function(options){
	...
	var d={
		...
		css:"body{margin:3px;font-family:verdana;font-size:25px;}p{margin:0px;}",
	...
	var keypress = function(e){
		...
		{
			fkey = e.keyCode;
			if(fkey==0 && e.altKey == false && e.ctrlKey == false)
				assamese(e); // Calling the function to generate the Assamese Character
			else
				return false;
		}
	...
	function assamese(e)
	{
		...
		//if(document.getElementById("radio1").checked == false)  return false;
		...
		if ((charCode >= 46 || charCode==36))
		{
			...
			{
				xmlDoc=new ActiveXObject("Microsoft.XMLHTTP");// IE 5 and IE 6
			}	
			xmlDoc.open("GET","js-assamese/assamese.xml",false);
			xmlDoc.send("");
			xmldoc1= xmlDoc.responseXML;
			nodename = "C" + charCode ;
			//alert(nodename);
			try
			{
				displaychar=xmldoc1.getElementsByTagName(nodename)[0].childNodes[0].nodeValue;
				//alert("Found:"+displaychar);
			}
			catch(err)
			{
				displaychar = charCode;
				//alert("Not Found:"+displaychar);
				//return false;
			}
		
			//alert(displaychar);
			//targ.value += String.fromCharCode(displaychar);
			//alert(String.fromCharCode(displaychar));
			text = String.fromCharCode(displaychar);
			//alert(text);
			/*if($.browser.msie===true && text == "~" || text == "@" || text == "#" || text == "%" || text == "&" || text == "*" || text == "+" || text == "=" || text == "|" )
			{
				//text = "<font face='Trebuchet MS'>"+ text +"</font>";
				text = "<span styleface='Trebuchet MS'>"+ text +"</font>";
			}*/

			//alert(text);
			insert_text(text); // Insert text into the textarea
			//insertHere(targ,displaychar);
	...
	this._init = function(is_init){
		...
		try {
		   ...
		   if(d.css.indexOf("background:")===-1){d.css+="body{background:white;}";}
		   if(d.css.indexOf("background-image:")===-1){
		       d.css=d.css+"body{background-image:url("+d.idir+"logo.gif);background-position:top right;background-repeat:no-repeat; border:0px;}";
		   }
		   
		   if($.browser.msie===true)
		   {
			   if(d.css.indexOf("@font-face")===-1)
			   {
				   d.css+="@font-face {font-family: asfont;font-style: normal;font-weight: normal;src: url(EKUSHEY0.eot);}";
				   d.css+="body{font-family: asfont;}";
				   //d.css +="body{font-family: asfont; font-family: Trebuchet MS}";
			   }
		   }
		   else
		   {
			   ...
			   if($.client.os=='Windows')
			   {
				   if(d.css.indexOf("@font-face")===-1)
				   {
					   ...
					   d.css+="@font-face {font-family: asfont;font-style:  normal;font-weight: normal; src: url('" + url + "asfont.otf') format('opentype');}";
					   //d.css+="@font-face {font-family: asfont; src: url('" +document.location.href+ "asfont.otf') format('opentype');}";
					   d.css+="body{font-family: asfont;}";
				   }
			   }
		   }
		   ...
		   if(iframe.contentWindow.document.attachEvent){
			   ...
			   //iframe.contentWindow.document.attachEvent("onkeydown", keypress);
			   iframe.contentWindow.document.attachEvent("onkeydown", fn);
		       //iframe.contentWindow.document.attachEvent("onkeyup", keyup);
		   }else{
			   iframe.contentWindow.document.addEventListener("keypress",keypress,false); //Added by me
			   //iframe.contentWindow.document.addEventListener("keydown",keypress,false); //Added by me
			   iframe.contentWindow.document.addEventListener("keydown",fn,false);
	...
	this.cmd = function(cmd,arg1){
	   ...
	   if(cmd==="code"){
		...
		var obj = d.iframe.contentWindow.document.body
		var speller = new spellChecker(textarea1, obj);
		speller.openChecker();	      
